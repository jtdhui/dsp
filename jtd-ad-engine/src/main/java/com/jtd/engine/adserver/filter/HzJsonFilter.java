...
import com.jtd.engine.message.v1.HzBidResponse;
 ...
 * @描述 <p>互众过滤</p>
 */
public class HzJsonFilter extends IoFilterAdapter {
	private static final String MZ_BID_URI = "/hzbid";

	private static final Log log = LogFactory.getLog(HzJsonFilter.class);
...
//					logMyDebug.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>请求开始<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
					//logMyDebug.info(json);
					HzBidRequest msg = null;
					try{
						msg = JSON.parseObject(json, HzBidRequest.class);
					}catch(Exception ex){
						ex.printStackTrace();
					}
	...
	public void filterWrite(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws Exception {
		if (interceptAddresses.contains(session.getLocalAddress())) {
			...
			if(message instanceof HzBidResponse) {
				
				HzBidResponse rsp = (HzBidResponse) message;
