...
import com.jtd.engine.message.v1.SaxBidRequest;
import com.jtd.engine.message.v1.SaxBidResponse;
 ...
 * @描述 <p></p>
 */
public class SaxJsonFilter extends IoFilterAdapter {

	private static final String SAX_BID_URI = "/saxb";

	private static final Log log = LogFactory.getLog(SaxJsonFilter.class);
	...
	public void messageReceived(NextFilter nextFilter, IoSession session, Object message) throws Exception {
		if (interceptAddresses.contains(session.getLocalAddress())) {
			...
			if(message instanceof HttpRequest){
				...
				if(req.getMethod() == HttpRequest.POST && SAX_BID_URI.equals(req.getUri())){
					byte[] body = req.getBody();
					String json = new String(body, 0, body.length, "UTF-8");
					SaxBidRequest msg = JSON.parseObject(json, SaxBidRequest.class);
	...
	public void filterWrite(NextFilter nextFilter, IoSession session,
		...
		if (interceptAddresses.contains(session.getLocalAddress())) {
			...
			if(message instanceof SaxBidResponse) {
