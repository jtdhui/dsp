...
import com.jtd.engine.message.v1.MMBidRequest;
import com.jtd.engine.message.v1.MMBidResponse;
 ...
 * @描述 <p></p>
 */
public class MediaMaxJsonFilter extends IoFilterAdapter {

	private static final String MM_BID_URI = "/mmb";

	private static final Log log = LogFactory.getLog(MediaMaxJsonFilter.class);
	...
	public void messageReceived(NextFilter nextFilter, IoSession session, Object message) throws Exception {
		if (interceptAddresses.contains(session.getLocalAddress())) {
			...
			if(message instanceof HttpRequest){
				...
				if(req.getMethod() == HttpRequest.POST && MM_BID_URI.equals(req.getUri())){
					byte[] body = req.getBody();
					String json = new String(body, 0, body.length, "UTF-8");
					MMBidRequest msg = JSON.parseObject(json, MMBidRequest.class);
	...
	public void filterWrite(NextFilter nextFilter, IoSession session,
		...
		if (interceptAddresses.contains(session.getLocalAddress())) {
			...
			if(message instanceof MMBidResponse) {
				
				MMBidResponse rsp = (MMBidResponse) message;
