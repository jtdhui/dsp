...
public abstract class AbstractDBSelectedShardedRedisDAO {
	...
	protected long remRetLong(String key){
		ShardedJedis sjedis = null;
		long ret = 0;
		try {
			sjedis = pool.getResource();
	...
	/**
	 * 向reids中写入两条数据，首先是kv1为key，kv2为value，过期时间为seconds，单位秒
	 *                     其次是kv2为key，kv1为value，过期时间为seconds，单位秒
	 * @param kv1
	 * @param kv2
	 * @param seconds
	 * @return void
	...
	/**
	 * 获取key对应的hash中所有的field，和field对应的value，放到Map中返回
	 * @param key
	 * @return
