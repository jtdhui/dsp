...
public class CampaignDAOImpl extends AbstractRedisDAO implements CampaignDAO {
	...
	private void setFretType(Campaign camp) {
		...
		if(dims == null) return;
		//拿到广告活动，频次和是否首屏的数据。{freq:[24,5,5], firsts: 0}
		...
		if(pv && click) {//如果pv和click都设置了，则key=活动id，value=[3,获取小时数据]
			freqType.put(camp.getId(), new int[]{3, freq.freq[0]});
		} else if(pv) {//如果pv设置了，则key=活动id，value=[1,获取小时数据]
			freqType.put(camp.getId(), new int[]{1, freq.freq[0]});
		} else if(click) {//如果click设置了，则key=活动id，value=[2,获取小时数据]
	...
	public int getPartnerGrossProfit(long partnerId) {
		//获取广告主的毛利率，如果广告主的毛利为null，则返系统默认的毛利率
	...
	public int[] getCampFreqType(long campid) {
		//获取广告活动的频次数据，返回的格式为  
		//如果pv和click都选了，[3,小时数据]，如果选pv [1,小时数据],如果选click[2,小时数据]
	...
	public static class FreqAndScreen {
		//对应的频次控制，长度为3的数组，格式为[界面上的小时数（24或者48），要控制的展现次数，要控制的点击次数]
		//界面上展现和点击是单选的，选择其中一个的时候，另外一个置0。
		private int[] freq;
		//取值0，1，2，分别对应不限，首屏，非首屏
		private int firsts;
