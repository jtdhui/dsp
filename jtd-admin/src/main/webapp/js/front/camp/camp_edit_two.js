...
function initOsType(){
	...
	// 操作系统
	if (osType) {
		$("#device :radio[name=osType][value=" + osType + "]").prop("checked", true);
	}else {
		//默认选中
		$("#osType_ALL").prop("checked", true);
	}
	...
	$("#osType_ANDROID").click(function() {
        ...
        var appSelectValue = $("#appSelectValue").val(); //当前选中值
	...
	$("#osType_IOS").click(function() {
        ...
        // 操作系统为IOS时且应用定向为应用包投放时,默认应用定向为不限
        var appSelectValue = $("#appSelectValue").val(); //当前选中值
        var appData = $("#appData").val();
...
function transChannelsWindow(){
    ...
    if(editStepStatus == "3"){
        ...
        $("#all_channel").css('display','none');
        $("#clean_channel").css('display','none');
    }
...
function peopleDirect(camp_id){
    ...
    // 人群定向事件
    $(".peopleContainer .crowd-header li").click(function() {
        if (!$(this).hasClass("on")) {
            $(".peopleContainer .crowd-header li").removeClass("on");
	...
	$("#clearResult").click(function() {
		...
		$("#ip input[type='checkbox']").prop("checked", false);
