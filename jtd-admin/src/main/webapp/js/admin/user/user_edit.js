...
/**初始化页面后,加载数据*/
function bindInitPage() {

    //初始化页面后,加载数据
    var partnerId=$("#partnerId").val();
    var userId = $("#id").val();//编辑用户的ID
    var roleId = $("#role-select").val();//编辑用户的角色
            ...
            if(result.success){
                ...
                $.each(result.partnerList, function (index, obj) {
                    ...
                    $.each(result.upList, function (index, vo) {
                        if(vo.partnerId ==  obj.id || roleId == 1 || roleId == 10){
                            flag = true;
                        }
                    });
                    ...
                    if(roleId == 1 || roleId ==10){ // 管理员角色不可编辑
                        disabled = 'disabled';
                    }

                    var key_s = "";
                    if(level>2 || (level == 2 && obj.pid > 1 && partnerId != obj.pid)){
                        key_s = "_"+obj.key_id;
                    }
...
function saveUser() {
    ...
    $('#user_submit').click(function(){
        ...
        $.ajax({
            url:"update.action?roleId="+role_val,
            type:"post",
            contentType:"application/json;charset=utf-8",
