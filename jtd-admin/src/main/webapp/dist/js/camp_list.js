			...
			$.ajax({
				...
				success:function(result){
					if(result.success){
						...
						warningTip(this,"操作失败！");
//						$(this).attr('data-target', '#warningTip');
//				        $('.tip').text("操作失败!");
					}
				}
			});
    ...
    $("#batch_set").click(function () {
        ...
        if (chkt < 1) {
           warningTip(this,'请至少选择一个活动');
            ...
            $(this).attr('data-target', '#batchSet');
        }
    });
...
function changeStatus(status) {
    ...
    if (chkt >= 1) {
    	...
    	warningTip($('.changeStatus').eq(status - 2),"请至少选择一个活动");
    }

}

function warningTip(obj,msg){
	$('.tip').text('请至少选择一个活动');
    $(obj).attr('data-target', '#warningTip');
}

function confirmTip(obj,yesFunc){
	
	$(obj).attr('data-target', '#delModal');
	//$("#button_yes").unbind("click");
	//$("#button_yes").bind("click",yesFunc);
}