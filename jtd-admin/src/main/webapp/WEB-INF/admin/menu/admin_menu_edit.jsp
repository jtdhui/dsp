	...
	<div class="main-container">
		...
		<div class="main-content">
			...
			<div class="page-content">
				...
				<div class="row">
					<div class="col-xs-12">
						<form id="form" class="form-horizontal" action="${baseurl }admin/menu/update.action" method="post">
							...
							<div class="form-group has-info">
								...
								<div class="col-xs-12 col-sm-5">
								<input type="hidden" name="parentId"  id="parentId" value="${po.parentId}"/>
								<select class="form-control" id="menu-select" onChange="setSelectVal()">
									<option value="0" <c:if test="${po.parentId==0 }">selected='selected'</c:if>>一级菜单</option>
									<c:forEach items="${poList }" var="vo">
										<option value="${vo.id }" <c:if test="${po.parentId==vo.id }">selected='selected'</c:if>>${vo.name }</option>
									</c:forEach>
								</select>
								</div>
								<div class="help-block col-xs-12 col-sm-reset inline"></div>
							...
							<div class="form-group has-info">
								...
								<div class="col-xs-12 col-sm-5">
									<span class="block input-icon input-icon-right"> <input
										type="text" id="sortString" name="sortString" class="width-100" value="${po.sortString }" >
									</span>
								</div>
								<div class="help-block col-xs-12 col-sm-reset inline"></div>
							</div>
