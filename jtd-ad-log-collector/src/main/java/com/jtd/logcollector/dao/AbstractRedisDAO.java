...
public abstract class AbstractRedisDAO {
	...
	protected Long setnx(String key, String value) {
		...
		try {
			...
			log.error("访问Redis SETNX出错, key:" + key + ",value:" + value, e);
			if (jedis != null) pool.returnBrokenResource(jedis);
		}
		return ret;
	}
	
	protected Long setnx(int dbIndex, String key, String value) {
		Jedis jedis = null;
		Long ret = null;
		try {
			jedis = pool.getResource();
	...
	protected void flushDB(int dbIndex) {
		Jedis jedis = null;
		try {
			jedis = pool.getResource();
			jedis.select(dbIndex);
